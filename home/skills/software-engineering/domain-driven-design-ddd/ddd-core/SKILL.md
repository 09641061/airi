---
name: ddd-core
description: Model a system with Domain-Driven Design end to end, language- and framework-agnostic — distill subdomains into core/supporting/generic, draw bounded contexts and a ubiquitous language, pick the context-mapping integration pattern (Partnership, Shared Kernel, Customer/Supplier, Conformist, ACL, OHS/Published Language, Separate Ways), model the tactical building blocks (entities, value objects, aggregates, domain services, domain events, repositories, factories), size aggregates with Vernon's four rules, wire the object lifecycle with a repository port/adapter and a thin application service, and choose a CQRS adoption level. Use when starting a DDD project, splitting a monolith or deciding microservice boundaries, arguing whether two features belong in the same context, integrating two contexts or a legacy system, deciding whether something is an entity or a value object, reviewing a model for anemia, fixing an aggregate that is too large or a transaction that touches two aggregates, designing repositories and application services, or deciding whether CQRS pays for itself.
---
